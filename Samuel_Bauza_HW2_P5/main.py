...
from robotplanner import robotplanner,initialize_planner
...
def runtest(mapfile, robotstart, targetstart):
  ...
  numofmoves = 0
  caught = False
  initialize_planner(env_map,robotpos,targetpos):
  for i in range(2000):
    # call robot planner
    t0 = tic()
    ...
    act_time = (tic()-t0)/2.0
    movetime = max(1, act_time)
    print('move time: %d (real) %f' %( movetime,act_time))
